...
void Mobile::PeriodicSimulationRenderer(const Environment& E,
                                double h,
                                Point X,
                                uint64_t seed,
                                std::string Reorientation_mode)
...
/*
Cette méthode graphique simule le processus à l'aide de l'algo de Mobile::simulation basé sur des sommes de Bernoulli,
En coordonnées sur le Tore */
...
// ---------------- GLFW INIT ----------------
    ...
    trajrenderer.addPoint(Point(0.0, 0.0));
    ...
    {
        step++;
        if ((Coord.getx()>E.getDomain().getradius()) || (Coord.gety()>E.getDomain().getradius()) ||(Coord.getx()<-(E.getDomain().getradius())) ||(Coord.gety()<-(E.getDomain().getradius())))
        {
            trajrenderer.clearTrajectory();
        }
        E.getDomain().MakeLoop(*this);
        //Coord est dans le domaine, Loop a compté le nombre de tours, et Free_coord est inchangé et encapsule les coordonnées "réelles"
        this->Theta += rotational_diffusivity(engine); // on affecte l'angle selon le coefficient Dr à chaque incrément temporel
        ...
        // on ajoute le point Coord au renderer de trajectoire
        trajrenderer.addPoint(Point(Coord));
